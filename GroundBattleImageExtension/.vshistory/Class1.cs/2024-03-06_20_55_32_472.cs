...
namespace DW2TroopsGroundBattleImage
{
    ...
    [HarmonyPatch(nameof(DistantWorlds.Types.UserInterfaceHelper.DrawTroopsCharactersIconized))]
    public class GroundTroopImagePatcher
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (var i = 0; i < codes.Count; i++)
            {
                ...
                {
                    codes[i] = CodeInstruction.Call(typeof(Troop), troopClass.GetType().GetProperty(nameof(troopClass.Definition)).GetSetMethod().Name, null, null);
